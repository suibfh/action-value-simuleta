// v011: Use static HTML inputs, full JS logic preserved
document.addEventListener('DOMContentLoaded', () => {
  ...
  function calculateTurnOrder() {
    ...
    for (let turn = 1; turn <= maxIterations; turn++) {
      ...
      for (let i = 0; i < unitCount; i++) {
        ...
        const currentEffects = effectMap[i]?.[turn];
        if (currentEffects) {
          if (currentEffects.type === 'agility_buff') cell.style.background = 'yellow';
          if (currentEffects.type === 'pressure') cell.style.background = 'lightblue';
          if (currentEffects.type === 'av_up') cell.style.background = 'lightgreen';
          if (currentEffects.type === 'av_down') cell.style.background = 'lightcoral';
        }
  ...
  function openEffectModal(unitIndex, turnNumber) {
    ...
    // Preset existing values if present
    const preset = effectMap[unitIndex]?.[turnNumber];
    if (preset) {
      typeSelect.value = preset.type;
      valueInput.value = preset.value;
      durationInput.value = preset.duration;
      targets.forEach(cb => cb.checked = preset.targets.includes(Number(cb.value)));
    }
